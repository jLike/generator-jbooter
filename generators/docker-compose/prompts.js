...
function askForApps() {
    ...
    this.prompt(prompts).then((props) => {
        ...
        this.appsFolders.forEach((appFolder) => {
            ...
            const config = fileData['generator-jbooter'];
...
function askForMonitoring() {
    ...
    const prompts = [{
        ...
        choices: [
            ...
            {
                value: 'elk',
                name: (this.composeApplicationType === 'monolith') ? 'Yes, for logs and metrics with the JBooter Console (based on ELK)' : 'Yes, for logs and metrics with the JBooter Console (based on ELK and Zipkin)'
            },
            {
                value: 'prometheus',
                ...
                try {
                    ...
                    if ((fileData['generator-jbooter'].baseName !== undefined)
                        && ((composeApplicationType === undefined)
                            ...
                            || ((composeApplicationType === 'microservice') && (fileData['generator-jbooter'].applicationType === 'gateway'))
                            || ((composeApplicationType === 'microservice') && (fileData['generator-jbooter'].applicationType === 'uaa')))) {
