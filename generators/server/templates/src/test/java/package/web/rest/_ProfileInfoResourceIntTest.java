...
public class ProfileInfoResourceIntTest <% if (databaseType === 'cassandra') { %>extends AbstractCassandraTest <% } %>{
    ...
    public void setup() {
        ...
        JBooterProperties.Ribbon ribbon = new JBooterProperties.Ribbon();
        ribbon.setDisplayOnActiveProfiles(mockProfile);
        when(jBooterProperties.getRibbon()).thenReturn(ribbon);

        String activeProfiles[] = {"test"};
        when(environment.getDefaultProfiles()).thenReturn(activeProfiles);
        when(environment.getActiveProfiles()).thenReturn(activeProfiles);

        ProfileInfoResource profileInfoResource = new ProfileInfoResource(environment, jBooterProperties);
