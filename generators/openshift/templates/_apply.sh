...
oc process -f <%-directoryPath%>/ocp/registry/jbooter-registry.yml | oc apply -f -
...
oc process -f <%-directoryPath%>/ocp/monitoring/jbooter-monitoring.yml | oc apply -f -
<%_ } _%> <%_ if (monitoring === 'prometheus') { _%>
oc process -f <%-directoryPath%>/ocp/monitoring/jbooter-metrics.yml | oc apply -f -
