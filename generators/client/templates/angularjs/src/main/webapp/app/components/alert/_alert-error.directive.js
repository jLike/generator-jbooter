...
(function() {
    ...
    var jbtAlertError = {
        template: '<div class="alerts" ng-cloak="" role="alert">' +
                        ...
                        '</div>' +
                  '</div>',
        controller: jbtAlertErrorController
    };

    angular
        .module('<%=angularAppName%>')
        .component('jbtAlertError', jbtAlertError);

    jbtAlertErrorController.$inject = ['$scope', 'AlertService', '$rootScope'<% if (enableTranslation) { %>, '$translate'<% } %>];

    function jbtAlertErrorController ($scope, AlertService, $rootScope<% if (enableTranslation) { %>, $translate<% } %>) {
        var vm = this;
