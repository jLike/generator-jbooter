...
const jbooterUtils = require('./util');
const constants = require('./generator-constants');

const JBOOTER_CONFIG_DIR = '.jbooter';
const MODULES_HOOK_FILE = `${JBOOTER_CONFIG_DIR}/modules/jbt-hooks.json`;
...
module.exports = class extends Generator {
    ...
    addElementToMenu(routerName, glyphiconName, enableTranslation, clientFramework) {
        ...
        try {
                ...
                jbooterUtils.rewriteFile({
                    ...
                    splicable: [`<li class="nav-item" routerLinkActive="active" [routerLinkActiveOptions]="{exact: true}">
                                <a class="nav-link" routerLink="${routerName}" (click)="collapseNavbar()">
                                    ...
                                    <span${enableTranslation ? ` jbtTranslate="global.menu.${routerName}"` : ''}>${_.startCase(routerName)}</span>
                                </a>
                            </li>`
                    ]
                }, this);
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + navbarPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + routerName} ${chalk.yellow('not added to menu.\n')}`);
        }
    }
    ...
    addElementToAdminMenu(routerName, glyphiconName, enableTranslation, clientFramework) {
        ...
        try {
                ...
                jbooterUtils.rewriteFile({
                    ...
                    splicable: [`<li>
                        <a class="dropdown-item" routerLink="${routerName}" routerLinkActive="active" (click)="collapseNavbar()">
                            ...
                            <span${enableTranslation ? ` jbtTranslate="global.menu.admin.${routerName}"` : ''}>${_.startCase(routerName)}</span>
                        </a>
                    </li>`
                    ]
                }, this);
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + navbarAdminPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + routerName} ${chalk.yellow('not added to admin menu.\n')}`);
        }
    }
    ...
    addEntityToWebpack(microserviceName, clientFramework) {
        ...
        jbooterUtils.rewriteFile({
            file: webpackDevPath,
            needle: 'jbooter-needle-add-entity-to-webpack',
    ...
    addEntityToMenu(routerName, enableTranslation, clientFramework) {
        ...
        try {
                    ...
                    splicable: [
                        this.stripMargin(
                            `|<li>
                             ...
                             |                            <span${enableTranslation ? ` jbtTranslate="global.menu.entities.${_.camelCase(routerName)}"` : ''}>${_.startCase(routerName)}</span>
                             |                        </a>
                             |                    </li>`
                        )
                    ]
            ...
            this.log(`${chalk.yellow('\nUnable to find ') + entityMenuPath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + routerName} ${chalk.yellow('not added to menu.\n')}`);
        }
    }
    ...
    addEntityToModule(entityInstance, entityClass, entityAngularName, entityFolderName, entityFileName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            jbooterUtils.rewriteFile({
                ...
                needle: 'jbooter-needle-add-entity-module-import',
                splicable: [
            ...
            jbooterUtils.rewriteFile({
                file: entityModulePath,
                needle: 'jbooter-needle-add-entity-module',
                splicable: [
                    this.stripMargin(
                        `|${appName}${entityAngularName}Module,`
                    )
            ...
            this.log(e);
            this.log(`${chalk.yellow('\nUnable to find ') + entityModulePath + chalk.yellow(' or missing required jbooter-needle. Reference to ') + entityInstance + entityClass + entityFolderName + entityFileName} ${chalk.yellow(`not added to ${entityModulePath}.\n`)}`);
        }
    }
    ...
    addAdminToModule(appName, adminAngularName, adminFolderName, adminFileName, enableTranslation, clientFramework) {
        ...
        try {
            ...
            jbooterUtils.rewriteFile({
                ...
                needle: 'jbooter-needle-add-admin-module-import',
                splicable: [
                    this.stripMargin(importStatement)
                ]
            }, this);
            ...
            jbooterUtils.rewriteFile({
                file: adminModulePath,
                needle: 'jbooter-needle-add-admin-module',
                splicable: [
                    this.stripMargin(
                        `|${appName}${adminAngularName}Module,`
                    )
            ...
            this.log(e);
            this.log(`${chalk.yellow('\nUnable to find ') + appName + chalk.yellow(' or missing required jbooter-needle. Reference to ') + adminAngularName + adminFolderName + adminFileName + enableTranslation + clientFramework} ${chalk.yellow(`not added to ${adminModulePath}.\n`)}`);
        }
    }
    ...
    addGlobalTranslationKey(key, value, language) {
        const fullPath = `${CLIENT_MAIN_SRC_DIR}i18n/${language}/global.json`;
        try {
            jbooterUtils.rewriteJSONFile(fullPath, (jsonObj) => {
                jsonObj[key] = value;
            }, this);
        } catch (e) {
     ...
     *
     * @param {string} javaDir - default java directory of the project (JBooter const)
    ...
    copyTemplate(source, dest, action, generator, opt = {}, template) {
        ...
        case 'stripHtml' :
            regex = new RegExp([
                /( (data-t|jbtT)ranslate="([a-zA-Z0-9 +{}'_](\.)?)+")/,                    // data-translate or jbtTranslate
            ...
            jbooterUtils.copyWebResource(source, dest, regex, 'html', _this, opt, template);
            break;
        case 'stripJs' :
            regex = new RegExp([
                /(,[\s]*(resolve):[\s]*[{][\s]*(translatePartialLoader)['a-zA-Z0-9$,(){.<%=\->;\s:[\]]*(;[\s]*\}\][\s]*\}))/, // ng1 resolve block
                /([\s]import\s\{\s?JhiLanguageService\s?\}\sfrom\s["|']ng-jbooter["|'];)/,       // ng2 import jbtLanguageService
                /(,?\s?JhiLanguageService,?\s?)/,                                                          // ng2 import jbtLanguageService
                /(private\s[a-zA-Z0-9]*(L|l)anguageService\s?:\s?JhiLanguageService\s?,*[\s]*)/,          // ng2 jbtLanguageService constructor argument
                /(this\.[a-zA-Z0-9]*(L|l)anguageService\.setLocations\(\[['"a-zA-Z0-9\-_,\s]+\]\);[\s]*)/, // jbtLanguageService invocations
            ].map(r => r.source).join('|'), 'g');

            jbooterUtils.copyWebResource(source, dest, regex, 'js', _this, opt, template);
            break;
        case 'copy' :
    ...
    /**
     * Register a module configuration to .jbooter/modules/jbt-hooks.json
     *
    ...
    registerModule(npmPackageName, hookFor, hookType, callbackSubGenerator, description) {
        try {
            ...
            let error;
            let duplicate;
            const moduleName = _.startCase(npmPackageName.replace(`${GENERATOR_JBOOTER}-`, ''));
            ...
            const moduleConfig = {
                ...
                npmPackageName,
                description: description || `A JBooter module to generate ${moduleName}`,
                hookFor,
                hookType,
                generatorCallback
            };
    ...
    getEntityJson(file) {
        ...
        try {
            entityJson = this.fs.readJSON(path.join(JBOOTER_CONFIG_DIR, `${_.upperFirst(file)}.json`));
        } catch (err) {
            this.log(chalk.red(`The JBooter entity configuration file could not be read for file ${file}!`) + err);
        }
    ...
    getExistingEntities() {
        ...
        if (shelljs.test('-d', JBOOTER_CONFIG_DIR)) {
            shelljs.ls(path.join(JBOOTER_CONFIG_DIR, '*.json')).forEach((file) => {
     ...
     */
    isJbooterVersionLessThan(version) {
        const jbooterVersion = this.config.get('jbooterVersion');
        if (!jbooterVersion) {
            return true;
        }
    ...
    checkForNewVersion() {
        try {
            ...
            shelljs.exec(`npm show ${GENERATOR_JBOOTER} version`, { silent: true }, (code, stdout, stderr) => {
                if (!stderr && semver.lt(packagejs.version, stdout)) {
                    ...
                    if (this.useYarn) {
                        this.log(chalk.yellow(`  Run ${chalk.magenta(`yarn global upgrade ${GENERATOR_JBOOTER}`)} to update.\n`));
                    } else {
                        this.log(chalk.yellow(`  Run ${chalk.magenta(`npm install -g ${GENERATOR_JBOOTER}`)} to update.\n`));
                    }
    ...
    updateLanguagesInLanguageConstantNG2(languages) {
        ...
        try {
            ...
            jbooterUtils.replaceContent({
                file: fullPath,
                pattern: /export.*LANGUAGES.*\[([^\]]*jbooter-needle-i18n-language-constant[^\]]*)\];/g,
                content
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. LANGUAGE constant not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
    ...
    updateLanguagesInWebpack(languages) {
        ...
        try {
            ...
            content +=
                '                        // jbooter-needle-i18n-language-webpack - JBooter will add/remove languages in this array\n' +
                '                    ]';

            jbooterUtils.replaceContent({
                file: fullPath,
                pattern: /groupBy:.*\[([^\]]*jbooter-needle-i18n-language-webpack[^\]]*)\]/g,
                content
            }, this);
        } catch (e) {
            this.log(chalk.yellow('\nUnable to find ') + fullPath + chalk.yellow(' or missing required jbooter-needle. Webpack language task not updated with languages: ') + languages + chalk.yellow(' since block was not found. Check if you have enabled translation support.\n'));
        }
    }
    ...
    template(source, destination, generator, options = {}, context) {
        ...
        jbooterUtils.renderContent(source, _this, _context, options, (res) => {
    ...
    checkGitConnection() {
        ...
        const done = this.async();
        exec('git ls-remote git://github.com/jbooter/generator-jbooter.git HEAD', { timeout: 15000 }, (error) => {
            if (error) {
