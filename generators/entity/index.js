...
const jbtCore = require('jbooter-core');
...
module.exports = EntityGenerator.extend({
    ...
    initializing: {
        ...
        getConfig() {
            ...
            this.jbtPrefix = this.config.get('jbtPrefix');
        ...
        validateEntityName() {
            ...
            } else if (!this.skipServer && jbtCore.isReservedClassName(this.name)) {
        ...
        validateTableName() {
            ...
            } else if (!this.skipServer && jbtCore.isReservedTableName(entityTableName, prodDatabaseType)) {
                this.warning(chalk.red(`The table name cannot contain the '${entityTableName.toUpperCase()}' reserved keyword, so it will be prefixed with 'jbt_'`));
                this.entityTableName = `jbt_${entityTableName}`;
    ...
    _loadJson() {
        ...
        if (jbtCore.isReservedTableName(this.entityTableName, this.prodDatabaseType)) {
            const entityTableName = this.entityTableName;
            this.entityTableName = `jbt_${entityTableName}`;
        }
        ...
        loadInMemoryData() {
            ...
            this.fields.forEach((field) => {
                ...
                if (_.isUndefined(field.fieldNameAsDatabaseColumn)) {
                    ...
                    if (jbtCore.isReservedTableName(fieldNameUnderscored, this.databaseType)) {
                        field.fieldNameAsDatabaseColumn = `jbt_${fieldNameUnderscored}`;
                    } else {
            ...
            this.relationships.forEach((relationship) => {
                ...
                if (otherEntityName === 'user') {
                    relationship.otherEntityTableName = 'jbt_user';
                } else {
                    ...
                    if (jbtCore.isReservedTableName(relationship.otherEntityTableName, this.prodDatabaseType)) {
                        const otherEntityTableName = relationship.otherEntityTableName;
                        relationship.otherEntityTableName = `jbt_${otherEntityTableName}`;
                    }
                }
