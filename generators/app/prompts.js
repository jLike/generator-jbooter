...
function askForMoreModules() {
    ...
    this.prompt({
        type: 'confirm',
        name: 'installModules',
        message: response => this.getNumberedQuestion('Would you like to install other generators from the JBooter Marketplace?', true),
        default: false
    }).then((prompt) => {
...
function askModulesToBeInstalled(done, generator) {
    generator.httpsGet('https://api.npms.io/v2/search?q=keywords:jbooter-module&from=0&size=50', (body) => {
