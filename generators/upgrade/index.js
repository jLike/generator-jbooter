...
const UPGRADE_BRANCH = 'jbooter_upgrade';
...
module.exports = UpgradeGenerator.extend({
    ...
    _cleanUp() {
        shelljs.ls('-A').forEach((file) => {
            if (['.yo-rc.json', '.jbooter', 'node_modules', '.git', '.idea'].indexOf(file) === -1) {
                shelljs.rm('-rf', file);
            }
        });
    ...
    _generate(version, callback) {
        this.log(`Regenerating application with JBooter ${version}...`);
        shelljs.exec('jbooter --with-entities --force --skip-install', { silent: this.silent }, (code, msg, err) => {
            if (code === 0) this.log(chalk.green(`Successfully regenerated application with JBooter ${version}`));
    ...
    _regenerate(version, callback) {
        this._generate(version, () => {
            ...
            this._gitCommitAll(`Generated with JBooter ${version}`, () => {
                callback();
            });
        });
    },
    ...
    configuring: {
        ...
        checkLatestVersion() {
            ...
            shelljs.exec(`${commandPrefix} ${GENERATOR_JBOOTER} version`, { silent: this.silent }, (code, msg, err) => {
                if (err) {
                    this.warning(`Something went wrong fetching the latest JBooter version number...\n${err}`);
        ...
        prepareUpgradeBranch() {
            ...
            const installJbooterLocally = (version, callback) => {
                this.log(`Installing JBooter ${version} locally`);
                const commandPrefix = this.clientPackageManager === 'yarn' ? 'yarn add' : 'npm install';
                shelljs.exec(`${commandPrefix} ${GENERATOR_JBOOTER}@${version} --dev --no-lockfile`, { silent: this.silent }, (code, msg, err) => {
            ...
            const regenerate = () => {
                this._cleanUp();
                installJbooterLocally(this.currentVersion, () => {
    ...
    default: {
        ...
        updateJbooter() {
            this.log(chalk.yellow(`Updating ${GENERATOR_JBOOTER} to ${this.latestVersion} . This might take some time...`));
            const done = this.async();
            const commandPrefix = this.clientPackageManager === 'yarn' ? 'yarn add' : 'npm install';
            shelljs.exec(`${commandPrefix} ${GENERATOR_JBOOTER}@${this.latestVersion} --dev --no-lockfile`, { silent: this.silent }, (code, msg, err) => {
                if (code === 0) this.log(chalk.green(`Updated ${GENERATOR_JBOOTER} to version ${this.latestVersion}`));
                else this.error(`Something went wrong while updating JBooter! ${msg} ${err}`);
                done();
            });
        },
