...
moveEntity() {
    local entity="$1"
    cp "$JBOOTER_SAMPLES"/.jbooter/"$entity".json "$APP_FOLDER"/.jbooter/
}
...
rm -Rf "$APP_FOLDER"
mkdir -p "$APP_FOLDER"/.jbooter/

if [ "$JBOOTER" == "app-ng2-mongodb" ]; then
...
elif [ "$JBOOTER" == "app-ng2-cassandra" ]; then
...
elif [[ ("$JBOOTER" == "app-mysql") || ("$JBOOTER" == "app-ng2-psql-es-noi18n") ]]; then
...
elif [ "$JBOOTER" == "app-ng2-gateway-uaa" ]; then
    moveEntity FieldTestEntity
    moveEntity FieldTestMapstructEntity
    moveEntity FieldTestServiceClassEntity
    moveEntity FieldTestServiceImplEntity
